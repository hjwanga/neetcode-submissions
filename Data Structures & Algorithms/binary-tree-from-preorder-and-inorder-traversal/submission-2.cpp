...
class Solution {
    ...
    // inorder: [in_l, in_r]
    TreeNode* dfs(const vector<int>& preorder, const unordered_map<int, int>& inorder_map, int pre_l, int pre_r, int in_l, int in_r) {
        ...
        int index_root = inorder_map.at(root_val);
        // 3. get left subtree size
        ...
        TreeNode* left = dfs(preorder, inorder_map, left_pre_l, left_pre_r, left_in_l, left_in_r);
        TreeNode* right = dfs(preorder, inorder_map, right_pre_l, right_pre_r, right_in_l, right_in_r);
        root->left = left;
        root->right = right;
        return root;
    }
public:
    TreeNode* buildTree(vector<int>& preorder, vector<int>& inorder) {
        int m = preorder.size();
        // <inorder_value, inorder_index>
        unordered_map<int, int> inorder_map;
        for (int i = 0; i < m; ++i) {
            inorder_map[inorder[i]] = i;
        }
        return dfs(preorder, inorder_map, 0, m-1, 0, m-1);
    }
};
